...
void updateWalls(struct coordinate point, int orient, bool L, bool R, bool F) {
	   if((L && R) && F){
		   if (orient==0){ cells[point.x][point.y]= 13; }//|-|
		   else if (orient==1){ cells[point.x][point.y]= 12; }//_-|
		   else if (orient==2){ cells[point.x][point.y]= 11; }//|_|
		   else if (orient==3){ cells[point.x][point.y]= 14; }//|-_
	   }
	   else if((L && R) && (!F)){
		   if (orient==0){ cells[point.x][point.y]= 9; }//| |
		   else if (orient==1){ cells[point.x][point.y]= 10; }//_-
		   else if (orient==2){ cells[point.x][point.y]= 9; }//| |
		   else if (orient==3){ cells[point.x][point.y]= 10; }//_-
	   }
	   else if((L && F) && (!R)){
		   if (orient==0){ cells[point.x][point.y]= 8; }//|-
		   else if (orient==1){ cells[point.x][point.y]= 7; }//-|
		   else if (orient==2){ cells[point.x][point.y]= 6; }//_|
		   else if (orient==3){ cells[point.x][point.y]= 5; }//|_
	   }
	   else if((R && F) && (!L)){
		   if (orient==0){ cells[point.x][point.y]= 7; }//-|
		   else if (orient==1){ cells[point.x][point.y]= 6; }//_|
		   else if (orient==2){ cells[point.x][point.y]= 5; }//|_
		   else if (orient==3){ cells[point.x][point.y]= 8; }//|-
	   	}
	   else if(F){
		   if (orient==0){ cells[point.x][point.y]= 2; }//-
		   else if (orient==1){ cells[point.x][point.y]= 3; }// |
		   else if (orient==2){ cells[point.x][point.y]= 4; }//_
		   else if (orient==3){ cells[point.x][point.y]= 1; }//|
	   	}
	   else if(L){
		   if (orient==0){ cells[point.x][point.y]= 1; }//|
		   else if (orient==1){ cells[point.x][point.y]= 2; }//-
		   else if (orient==2){ cells[point.x][point.y]= 3; }// |
		   else if (orient==3){ cells[point.x][point.y]= 4; }//_
	   	}
	   else if(R){
		   if (orient==0){ cells[point.x][point.y]= 3; }// |
		   else if (orient==1){ cells[point.x][point.y]= 4; }//_
		   else if (orient==2){ cells[point.x][point.y]= 3; }//|
		   else if (orient==3){ cells[point.x][point.y]= 4; }//-
	   	}
	    else{
...
bool isAccessible(struct coordinate p, struct coordinate p1) {
    if(p1.x<0 || p1.y<0 || p1.x>=16 || p1.y>=16){
    	return false;
    }

    if (p.x==p1.x){
        if(p.y>p1.y){
            if(cells[p.x][p.y]==4 || cells[p.x][p.y]==5 || cells[p.x][p.y]==6 || cells[p.x][p.y]==10 || cells[p.x][p.y]==11 || cells[p.x][p.y]==12 || cells[p.x][p.y]==14 ){
            	return false;
            }
            ...
            if(cells[p.x][p.y]==2 || cells[p.x][p.y]==7 || cells[p.x][p.y]==8 || cells[p.x][p.y]==10 || cells[p.x][p.y]==12 || cells[p.x][p.y]==13 || cells[p.x][p.y]==14 ){
            	return false;
            }
        ...
        if(p.x>p1.x){
            if(cells[p.x][p.y]==1 || cells[p.x][p.y]==5 || cells[p.x][p.y]==8 || cells[p.x][p.y]==9 || cells[p.x][p.y]==11 || cells[p.x][p.y]==13 || cells[p.x][p.y]==14 ){
                return false;
            }
            else{
                return true;
            ...
            if(cells[p.x][p.y]==3 || cells[p.x][p.y]==6 || cells[p.x][p.y]==7 || cells[p.x][p.y]==9 || cells[p.x][p.y]==11 || cells[p.x][p.y]==12 || cells[p.x][p.y]==13 ){
                return false;
            }
            else{
                return true;
...
void backtrack() {
	...
	while(!q.isempty()){
		...
		while(!q.isempty()){
			...
			if (surr.N.x>=0 && surr.N.y>=0 && cells[surr.N.x][surr.N.y]>=0 && backFlood[surr.N.x][surr.N.y]==-1){
				if(isAccessible(p, surr.N)){
					temq.enQueue(surr.N);
					backFlood[surr.N.x][surr.N.y] = distance;
				}
			}
			if (surr.E.x>=0 && surr.E.y>=0 && cells[surr.E.x][surr.E.y]>=0 && backFlood[surr.E.x][surr.E.y]==-1){
				if(isAccessible(p, surr.E)){
					temq.enQueue(surr.E);
					backFlood[surr.E.x][surr.E.y] = distance;
				}
			}
			if (surr.S.x>=0 && surr.S.y>=0 && cells[surr.S.x][surr.S.y]>=0 && backFlood[surr.S.x][surr.S.y]==-1){
				if(isAccessible(p, surr.S)){
					temq.enQueue(surr.S);
					backFlood[surr.S.x][surr.S.y] = distance;
				}
			}
			if (surr.W.x>=0 && surr.W.y>=0 && cells[surr.W.x][surr.W.y]>=0 && backFlood[surr.W.x][surr.W.y]==-1){
				if(isAccessible(p, surr.W)){
					temq.enQueue(surr.W);
