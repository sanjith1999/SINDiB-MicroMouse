...
static uint8_t currentState = 0; // Initial state is Idle
...
int main(void) {
	...
	while (1) {
		...
		case 0: // Idle
			...
			break;
		case 1: // Search
			// Perform Search state actions
			SearchRun();
			currentState = 0;
			break;
		case 2: // Fast
			// Perform Fast state actions
			FastRun();
			currentState = 0;
			break;
		case 3: // Speed Adjustment
			// Perform Speed Adjustment state actions
			AdjustSpeed();
			break;
		}
		/* USER CODE BEGIN 3 */
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOC,
			RF_EMITTER_Pin | D_C_Pin | DIAGONAL_EMITTER_Pin | LF_EMITTER_Pin
					| CS_Pin | LED10_Pin | LED3_Pin | LED4_Pin | LED5_Pin,
			GPIO_PIN_RESET);
	...
	/*Configure GPIO pins : RF_EMITTER_Pin D_C_Pin DIAGONAL_EMITTER_Pin LF_EMITTER_Pin
	 CS_Pin LED10_Pin LED3_Pin LED4_Pin
	 LED5_Pin */
	GPIO_InitStruct.Pin = RF_EMITTER_Pin | D_C_Pin | DIAGONAL_EMITTER_Pin
			| LF_EMITTER_Pin | CS_Pin | LED10_Pin | LED3_Pin | LED4_Pin
			| LED5_Pin;
	...
	/*Configure GPIO pins : BOOT1_Pin INT_Pin LENCHB_Pin BOOT0_Pin */
	GPIO_InitStruct.Pin = BOOT1_Pin | INT_Pin | LENCHB_Pin | BOOT0_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
